...
# Load your model
model_path = 'D:/Tissue Engineering/Main/Models/826-Precision-New-1024_vgg16_unet_w0.hdf5'
...
# bscan = np.zeros((36, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), dtype=np.uint8)
originalPath = 'D:/Tissue Engineering/Main/Data/Mine-Old/B10'
image_folder = os.listdir(originalPath)

i = 0
for x in image_folder:
    print(x)
    image_path = os.path.join(originalPath, x)  # Construct the full image path
    gray = cv2.imread(image_path, 0)
    gray = np.stack((gray,) * 3, axis=-1)
    if gray.shape != (IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS):
        gray = cv2.resize(gray, (IMG_WIDTH, IMG_HEIGHT))
    bscan = np.array(gray).reshape((IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
    
    predicted_mask = model.predict(np.expand_dims(bscan, axis=0))[0]    
...
for i in range(len(tracks)):
    ...
    if len(tracks[i])>=2:
        ...
        # tracks[i].append([surface_area, len(tracks[i]), surface_roughness, elongation, flatness])
...
df.to_csv('Results/csv/test-mscan.csv', index=False, header=None)